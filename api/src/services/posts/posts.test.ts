...
describe('posts', () => {
  ...
  scenario('creates a post', async () => {
    const result = await createPost({
      input: { title: 'String967766', body: 'String', tags: 'String' },
    })

    expect(result.title).toEqual('String967766')
  ...
  scenario('updates a post', async (scenario: StandardScenario) => {
    ...
    const result = await updatePost({
      id: original.id,
      input: { title: 'String74487462' },
    })

    expect(result.title).toEqual('String74487462')
  })
